...
def get_mask_card_number(card_number: str) -> str:
    ...
    """
    logger.info(f"Выполняем проверку на переданный номер карты {card_number}")
    if card_number is None:
        raise ValueError("номер карты не может быть None")

    logger.info(f"Выполняем проверку на 16-ти значный номер карты {card_number}")
...
def get_mask_account(okaunt_number: str) -> str:
    ...
    """
    logger.info(f"Выполняем проверку на переданный номер счета{okaunt_number}")
    if okaunt_number is None:
        raise ValueError("номер счета не может быть None")

    logger.info(f"Выполняем проверку на 20-ти значный номер карты {okaunt_number}")
    if (len(okaunt_number)) != 20 or not okaunt_number.isdigit():
        raise ValueError("номер счета должен состоять и 20 цифр")

    logger.info(f"Маскируем счет {okaunt_number}")
